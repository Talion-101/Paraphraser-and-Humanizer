            ...
            for synset in wordnet.synsets(word, pos=wordnet_pos):
                for lemma in synset.lemmas():
                    ...
                    if (synonym.lower() != word.lower() and 
                        ...
                        len(synonym) <= len(word) and  # STRICTER: Must be shorter or equal
                        len(synonym) < 10 and  # STRICTER: Max 10 chars
                        ' ' not in synonym and  # Single words only
                        len(synonym) > 2):  # At least 3 chars
                        
                        # Tense/Plural check heuristic
                        # If original ends in 'ed', synonym should probably end in 'ed'
                        if word.endswith('ed') and not synonym.endswith('ed'):
                            continue
                        # If original ends in 'ing', synonym should probably end in 'ing'
                        if word.endswith('ing') and not synonym.endswith('ing'):
                            continue
                        # If original ends in 's' (and isn't 'is'/'was'), synonym should probably end in 's'
                        if word.endswith('s') and not word.endswith('ss') and not synonym.endswith('s'):
                            continue
        ...
        for sentence in sentences:
            ...
            for word, pos in pos_tags:
                ...
                if word.lower() in self.stop_words or not word.isalpha() or len(word) < 4:
                    ...
                    # IMPROVED SCALING: 
                    # Even at intensity 0.1, we want ~25% chance of replacement for valid words
                    # At intensity 1.0, we want ~90% chance
                    base_chance = 0.2 + (intensity * 0.7)
...
class SemanticValidator:
    ...
    def _reincorporate_missing_terms(self, paraphrased_text, missing_terms):
        ...
        # DISABLED: This feature was inserting words randomly and breaking grammar
        # for sentence in sentences:
        #     modified = sentence
